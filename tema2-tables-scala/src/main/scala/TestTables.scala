...
object TestTables {
  ...
  def programmingLanguages1: Table = {
    val functional = NewCol("Functional", "Yes", Value(TestTables.tableFunctional))
    val imperative = NewCol("Imperative", "Yes", Value(TestTables.tableImperative))
    val objectOriented = NewCol("Object-Oriented", "Yes", Value(TestTables.tableObjectOriented))

    val result1 = Merge("Language", functional, objectOriented)
    val result2 = Merge("Language", result1, imperative)
  ...
  val programmingLanguages2: Table = {
    ...
    val result = Filter(
      And(Field("Original purpose", _.contains("Application")),
